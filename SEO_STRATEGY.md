...
SEO/AEO pass 2026-08-23 (post-redesign):

- SoftwareApplication structured data added: full block on the homepage (name matches the App Store listing, all four pricing offers, featureList) plus a lean offers-only block on volleyball-referee-app.html; homepage also gained a WebSite node, with all publisher references linked to the Organization node via `@id`.
- llms.txt gained an "About VolleyRef" quotable product-facts section. NOTE: llms.txt is generated by the private sportsref_aeo repo; port the section into that generator before its next sync. Until then, validate-site.mjs fails if the section disappears.
- Sitewide repair of broken punctuation left by the historical em-dash strip (~56 sentences across 17 pages read as fragments, e.g. "The libero can serve. but only..."). A new validate-site.mjs gate now blocks the artifact class; beachtennisref still has ~440 (gate off there until swept).
- Offline FAQ on volleyball-referee-app.html corrected to the true offline-first behavior, with the JSON-LD answer matching the visible text.
- Sitemap lastmod bumped for all touched pages.
