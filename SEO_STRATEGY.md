...
SEO/AEO pass 2026-08-23 (post-redesign): SoftwareApplication + WebSite + VideoObject structured data added to the homepage and SoftwareApplication to volleyball-referee-app.html (name matches the App Store listing, all four pricing offers, featureList); llms.txt gained an "About VolleyRef" quotable product-facts section (NOTE: llms.txt is generated by the private sportsref_aeo repo, so this section must be ported into that generator before its next sync or it will be overwritten); sitewide repair of broken punctuation left over from the em-dash strip (about 50 sentences across 16 pages read as fragments, e.g. "The libero can serve. but only..."); offline FAQ on volleyball-referee-app.html corrected to the true offline-first behavior; sitemap lastmod bumped for all touched pages.
